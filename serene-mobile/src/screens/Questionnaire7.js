...
const Questionnaire7 = ({ navigation, route }) => {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      scores: { q1: s1, q2: s2, q3: s3, q4: s4, q5: s5, q6: s6, q7: s7 },
    ...
    try {
      ...
      if (response.data) {
        // We use replace so the user can't "Go Back" to Q7 easily
      ...
      if (serverMessage && serverMessage.includes("already logged")) {
        Alert.alert(
          "Already Logged", 
...
const styles = StyleSheet.create({
  ...
  screenTitle: { fontSize: 22, color: "#512DA8", fontFamily: "Quicksand-Bold", marginBottom: 20 },
  questionCard: { backgroundColor: "#FFFFFF", borderRadius: 15, padding: 20, width: "100%", elevation: 3 },
  questionText: { fontSize: 16, color: "#333", fontFamily: "Quicksand-SemiBold", marginBottom: 20 },
  optionsContainer: { width: "100%" },
  option: { padding: 10, borderRadius: 10, backgroundColor: "#F8F9FE", marginBottom: 10, borderWidth: 1, borderColor: "#E8E3F9" },
  ...
  bottomButtons: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", width: "95%", marginTop: 30, paddingBottom: 10 },
