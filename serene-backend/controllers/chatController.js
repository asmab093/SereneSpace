...
exports.getChatHistory = async (req, res) => {
  try {
    ...
    let history = await Message.find({ user: req.user.id }).sort({
      createdAt: 1,
    });
...
exports.getChatResponse = async (req, res) => {
  ...
  try {
    ...
    const isMentalHealthIssue = ["stress", "anxiety", "depression"].includes(
      lowerCategory,
    );
    ...
    if (isCrisis) {
      ...
      const scenarioType = isMentalHealthIssue
        ? "CRISIS_WITH_CATEGORY"
        : "CRISIS_ONLY";
      const crisisReply =
        "I'm deeply concerned by what you're sharing. Please stay with me and use the resources below.";
      ...
      if (recentCrisis) {
        console.log(
          "⏳ Cooldown active: Emergency alert was already sent within the last 15 minutes. Skipping WhatsApp API.",
        );
      } else {
        ...
        try {
          ...
          if (
            ...
            console.log(
              `✅ WhatsApp Alert successfully sent to ${fullPhoneNumber}`,
            );
            smsSuccessfullySent = true;
          } else {
            console.log(
              "⚠️ Crisis detected, but user has no emergency contact saved.",
            );
          }
        } catch (apiError) {
          console.error(
    ...
    const basePrompt = `You are SereneBot, a compassionate and concise mental health AI operating in Pakistan. 
    CRITICAL LENGTH INSTRUCTION: Keep all responses brief, warm, and engaging. Do not write long paragraphs; limit 
    ...
    if (isMentalHealthIssue) {
      systemPrompt =
        basePrompt +
        `The user is feeling ${lowerCategory}. Use CBT techniques to challenge negative thoughts, give 3 actionable tips, and end with an empathetic question.`;
    } else {
      systemPrompt =
        basePrompt +
        `The user is feeling okay. Maintain a supportive, lighthearted conversation.`;
    }
    ...
    const botReply =
      chatCompletion.choices[0]?.message?.content || "I'm here for you.";
...
exports.transcribeAudio = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Speech recognition failed." });
  }
};
